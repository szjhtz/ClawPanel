...
> 这套 `defaultAccount + accounts + 顶层镜像` 是 **ClawPanel 当前的统一配置模型**。  
> 对外部飞书官方早期版插件而言，公开资料（官方文档和字段说明）目前能稳定证实的是顶层共享字段；`accounts` / `defaultAccount` / `botName` / `enabled` 这类高级账号结构是否被插件运行时完整消费，仍要以插件版本和实测结果为准。
...
如果你走的是**飞书官方早期版插件**路线，建议优先保证：

1. 默认账号具备完整 `appId` / `appSecret`（面板会自动将其镜像到顶层，无需手动维护顶层字段）
2. 生产稳定性不要建立在“官方插件一定完整消费 `accounts/defaultAccount/botName/enabled`”这个未经完全证实的假设上
